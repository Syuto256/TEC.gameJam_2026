...
| **`Archive/`** | **終わった計画の保管庫。読まない** | 計画の完了・撤回時に移す |

> **`Archive/` は判断に使いません。** 書かれた時点では正しくても、その後の決定で覆っている記述が
> 混ざっています。作業の判断は `Architecture/` `Specifications/` `Decisions/` の 3 つで行います。
> 詳しくは [保管庫](Archive/README.md) を参照。

# GameManager（Game シーンの配線）

最終更新: 2026-08-04  
実装: `Assets/Scripts/Core/GameManager.cs`  
状態: 実装済み・Scene 配線済み。画面構成の合意内容は [Game 画面レイアウト案](../GameDesign/game-screen-layout.md) を正とする。

旧称は `GameSceneUiReferences`。シーンごとの入口を `<シーン名>Manager` にそろえるため改名した（[シーン構造](scene-structure.md)）。

## 責務

`Game.unity` に常設する View とゲーム進行コンポーネントを接続する。個々のウィジェット（テキスト、バー、ボタン）は各 View が Inspector で保持し、このクラスは **View と Controller をつなぐ以外の責務を持たない**。

ゲーム進行そのものは `MainGameController` が持つ。「配線係が `GameManager`、進行が `MainGameController`」という分担である。
...
- `Awake` を使わないのは、`PausePanel` のように**非表示状態で開始する枝に置かれた View では `Awake` が走らない**ためである。配線の実行順を `GameManager` が決める形にして、この事故を構造的に防ぐ。
- 検証は `SceneUiValidation.Require` に集約し、不足しているフィールド名を列挙して報告する。
- `GameManager` は 4 つの `Initialize()` を短絡しない `&` で評価し、不足を一度にすべて報告する。

## Inspector で保持する参照

| 区分 | 参照 |
| --- | --- |
| 設定 | `GameTuningSettings` |
...
デバイス面は個別フィールドではなく配列で持つ。3 つ目の面を足す場合は Variant を 1 つ作って配列へ追加するだけで、`GameManager` も `DeviceScreenController` も `MainGameController` も変更しない。配列の `Surface` 重複と未設定要素は開始時に検出して停止する。

左右のタスク生成領域は `DeviceWorkspaceView.PickSpawnArea()` が持ち、吹き出しの少ない側を返す。`MainGameController` は `Surface` から面を引いてこれを呼ぶだけで、左右を判断しない。

ミニゲームの登録簿は `MainGameController` 側の `miniGameCatalog` が持つ。追加手順は [ミニゲームの追加・改造手順](mini-game-catalog.md) を参照する。

## 配置上の注意

- `GameManager` / `MainGameController` / `DeviceScreenController` は、`Game.unity` の `GameManager` GameObject に付ける。
...
- 実行時に生成するのはタスク吹き出しとミニゲーム Prefab、およびなぞりのガイド線のみである。

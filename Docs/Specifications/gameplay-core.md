...
| 結果 | HP | スコア |
| --- | ---: | --- |
| 自力成功 | 変化なし | `基礎点 × (1 + 0.5 × 残り寿命比) × コンボ倍率` |
...
### コンボ（2026-08-04 実装・採用）

自力成功が連続した件数を `GameSession.ComboCount` が持つ。自力失敗・AI 失敗・未着手時間切れで 0 に戻る。

| 項目 | 初期値 | 所有者 |
| --- | ---: | --- |
| コンボ 1 段あたりの倍率上乗せ | 0.10 | `GameTuningSettings.score.comboScoreAddPerCombo` |
| コンボ倍率の上限 | 2.00 | `GameTuningSettings.score.maxComboMultiplier` |
| 効果音を鳴らすコンボ間隔 | 10 | `GameTuningSettings.score.comboMilestoneInterval` |

コンボ倍率は `1 + 加算値 × (コンボ数 - 1)` を上限でクランプした値である。1 コンボ目は等倍のため、コンボを切らしても「元に戻る」だけで、マイナスにはならない。企画書 8.3 の「コンボ切れでやる気を削がない」という懸念は、加算方向のみのボーナスとすることで満たしている。

**TODO(2026-08-04):** AI 成功はコンボを伸ばさないが、その時点のコンボ倍率は受け取る。自力と AI の評価差（企画書 22.5）の決定に合わせて、どちらへ寄せるか確定する。

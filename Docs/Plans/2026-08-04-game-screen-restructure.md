...
- ミニゲーム中に `DeviceScreenController.SetSwitchEnabled(false)` を呼ぶ接続は未了。
...
- タスク吹き出しの親を、選択中デバイス側の左右 `TaskSpawnArea` に振り分ける。（未了。現在は `LeftTaskArea` にのみ生成する）
...
- 背景、端末枠、タスク領域、HUD、タブ、モーダルを個別に Prefab 化する範囲を決める。（タスク吹き出しのみ先行して Prefab 化済み）
- 素材差し替え手順と、Scene 上で調整する項目を資料化する。（吹き出しは [クラスカタログ](../Architecture/class-catalog.md) の「タスク吹き出しの調整場所」に記載済み）

...
### 7 の検証（§3-1 の反省を適用）— 2026-08-07 実施

**「チュートリアルをわざと失敗しながら最後まで通す」を必ず実行する。**
前回の進行不能バグはこの経路でしか出なかった。成功しながら通しても見つからない。

- [x] 全 34 ステップを強制送りなしで完走（計測ログで確認。F9 の使用 0 回）
- [x] **わざと失敗しても進行が止まらないこと**
- [x] チュートリアル完了後、本編へ正しく遷移すること
- [x] 通常の難易度で `TutorialOverlay` が非アクティブであること
- [x] `GameTuningSettings.tutorial` の 5 項目が 0 でないこと（読み直して確認）
- [x] EditMode テスト 73/73
- [x] Console エラー 0 件・全 5 シーンの欠損スクリプト 0 件

> **注意（実際に踏んだ）**: テスト中に `TutorialOverlay.activeSelf` が `True` のまま
> `Game.unity` に保存されていた。**通常プレイでチュートリアル UI が出る状態**になる。
> 再生を挟んだ後は、この値を必ず確認すること。
